...
main.config(bg="#fff")
#logo = tk.PhotoImage(file = "images/DRBIKE_LOGO.gif")
...
class BusLog:
    ...
    def next_bus(self, minutes):
        ...
        self.canvas.create_text(x, y, anchor = tk.NW, text = f"Next Order in {round(minutes, 1)} minutes")
    ...
    def bus_arrived(self, people):
        x = self.x_top + 135
        y = self.y_top + (self.bus_count * self.TEXT_HEIGHT)
        self.canvas.create_text(x, y, anchor = tk.NW, text = f"Order for {people} people", fill = "green")
        self.bus_count = self.bus_count + 1
        self.canvas.update()
