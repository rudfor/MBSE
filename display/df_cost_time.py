...
def number_of_deliveries(bike, drone): # input: list of all orders for both bikes and drones also missed deliveries
    ...
    missed_bike_orders = []
    drone_orders_1 = [i[1] for i in drone if i[0]==4]
    drone_orders_2 = [i[1] for i in drone if i[0]==5]
    drone_orders_3 = [i[1] for i in drone if i[0]==6]
    ...
    drone_total_order = last_drone_1 + last_drone_2 + last_drone_3
    ...
    orders_in_total = last_bike + drone_total_order
    ...
    drone_percent = drone_1_percent + drone_2_percent + drone_3_percent
    orders_percent = bike_percent + drone_1_percent + drone_2_percent + drone_3_percent
    declined_orders_percent = round((declined_orders/orders_in_total)*100) if declined_orders > 0 else 0

    label = ['Bicycle', 'Dronetype 1', 'Dronetype 2', 'Dronetype 3', 'Drone total', 'Declined', 'Total']
    data_orders = [last_bike, last_drone_1, last_drone_2, last_drone_3, drone_total_order, declined_orders, orders_in_total]
    data_percent = [bike_percent, drone_1_percent, drone_2_percent, drone_3_percent, drone_percent,declined_orders_percent, orders_percent]
...
def transit_time_distance(bike, drone):# Input: bike_time, drone_time, -> simulator give the time it takes for each to reach distination
    ...
    range_bike = [i*0+1000 if i<=1000 else i*0+2000 if i > 1000 and i <= 2000 else i*0+3000 if i > 2000 and i <= 3000 else i*0+4000 if i > 3000 and i <= 4000 else i*0+5000 for i in bike_transit_distance]

    drone_transit_time = [i[1] for i in drone]
    drone_transit_distance = [i[1]*drone1.avg_speed if i[0]==4 else i[1]*drone2.avg_speed if i[0]==5 else i[1]*drone3.avg_speed for i in drone]
    drone_range = [i*0+1000 if i<=1000 else i*0+2000 if i > 1000 and i <= 2000 else i*0+3000 if i > 2000 and i <= 3000 else i*0+4000 if i > 3000 and i <= 4000 else i*0+5000 for i in drone_transit_distance]
...
def bike_cost(data, time):
    ...
    delivery_wage = (bike.cost_hour/60)*time*employees#TIME_LIMIT_MINUTES #wage in minutes; wage in minutes*time of simulation*number of employees 
